...
void run_sim(double *u, double *u0, double *u1, double *pebbles, int n, double h, double end_time)
{
  ...
  int z=0;
  double* uo_t=uo, *uc_t=uc, *u0_t=u0, *u1_t=u1;
  
  #pragma omp parallel for private(z) num_threads(nthreads)
  for(z=0;z<n;z++)
  {
    memcpy(uo_t+(z*n), u0_t+(z*n), sizeof(double) * n);
    memcpy(uc_t+(z*n), u1_t+(z*n), sizeof(double) * n);
  }
  ...
  #pragma acc data copyin(pebbles[:n*n],uc[:n*n],uo[:n*n]) copyout(un[:n*n])
  /* loop until time >= end_time */
  while(1)
  {
    #pragma acc kernels loop present(un[:n*n],uc[:n*n],uo[:n*n],pebbles[:n*n])
	...
	//printf("total threads is %d \n", omp_get_num_threads());
        ...
        {
            #pragma acc cache(pebbles[idx-1:3],uc[idx-1:3],uc[idx-n-1:3],uc[idx+n-1:3])
